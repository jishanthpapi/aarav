...
function Row({ label, value, tone = 'normal' }: {
  ...
  return (
    <div className="flex items-baseline justify-between gap-3">
      ...
      <span className={`font-mono text-xs tabular-nums ${colour}`}>{value}</span>
    </div>
  );
}
...
export function AccuracyPanel({ plan, re, conv, cd }: {
  ...
  return (
    <div className="w-full space-y-3 rounded-xl border border-white/10 bg-black/70 p-4 backdrop-blur-md">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-medium text-white/80">
          {conv.developing ? 'Flow developing'
            : conv.converged ? 'Statistically settled'
            : 'Averaging'}
        </span>
        <span className="ml-auto font-mono text-xs text-white/35">{conv.stepsRun} steps</span>
      </div>
      ...
      <div className="space-y-1 border-t border-white/10 pt-3">
        ...
        />
        <Row label="Grid" value={`${plan.grid.nx} × ${plan.grid.ny} × ${plan.grid.nz}`} />
        <Row label="Total cells" value={(plan.grid.nx * plan.grid.ny * plan.grid.nz).toLocaleString()} />
        <Row label="Cell spacing" value={`${(plan.grid.dx * 100).toFixed(1)} cm`} />
        <Row
          label="Blockage"
      ...
      {plan.warnings.map((w) => (
        <div key={w} className="flex gap-2 rounded-lg border border-amber-500/20 bg-amber-500/5 p-2">
          ...
          <p className="text-xs leading-relaxed text-white/55">{w}</p>
        </div>
      ))}
      ...
      <div className="flex gap-2 rounded-lg border border-white/10 bg-white/[0.03] p-2">
        ...
        <p className="text-xs leading-relaxed text-white/45">
          ...
          not its distance from reality. This grid may miss thin parts and narrow
          ground gaps. Boundary layers and separation remain under-resolved, and
          aerodynamic trends still need independent validation.
        </p>
      </div>
    </div>
  );

...
export default function AaravChat() {
  ...
  const send = async () => {
    ...
    try {
      for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        const snapshot = snapshotSceneState();
        const response = await sendAaravMessage(history, snapshot);
        if (JSON.stringify([snapshotSceneState().vehicleId, snapshotSceneState().slotValues, snapshotSceneState().windSpeedKph]) !== JSON.stringify([snapshot.vehicleId, snapshot.slotValues, snapshot.windSpeedKph])) throw new Error("The scene changed while Aarav was responding. Please ask again.");
      ...
      throw new Error("Tool limit reached. Review the current scene before continuing.");
    } catch (err) {
  ...
  if (!open) return <button className="absolute bottom-4 left-4 rounded-lg bg-blue-700 p-3 text-xs text-white z-30" onClick={() => setOpen(true)}>Ask Aarav</button>;
  return (
    <div className="absolute bottom-6 left-4 w-64 bg-[#0f0f0f] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-20">
      <div className="p-3 border-b border-white/10 bg-white/5 flex items-center gap-2">
        ...
        <button className="ml-auto text-xs text-white/60" onClick={() => setOpen(false)}>Close</button>
      </div>
